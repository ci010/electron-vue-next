...
program
  ...
  .option('-ev, --enable-vitepress', 'generate vitepress for document')
  ...
  .on('option:enable-vitepress', () => { provided.vitepress = true })
  ...
  .action(() => {
    ...
    options.vitepress = opts.enableVitepress
...
async function interactive(name) {
  const { projectName, nodeIntegration, mainService, vscode, vitepress } = await inquirer.prompt([
    ...
    { type: 'confirm', default: false, message: 'Use vitepress for document:', name: 'vitepress', when: !provided.vitepress }
  ])
  options.projectName = projectName
  options.nodeIntegration = nodeIntegration
  options.mainService = mainService
  options.vscode = vscode
  options.vitepress = vitepress
}
...
async function setupProject() {
  ...
  await copy(srcDir, distDir, {
    ...
    filter: (src, dest) => {
      ...
      if (!options.vitepress) {
        ...
        if (relativePath.startsWith(join('scripts', 'dev.docs.js'))) {
          return false
        }
      }
      return true
  ...
  if (!options.vitepress) {
    delete packageJSON.optionalDependencies.vitepress
    delete packageJSON.scripts['dev:docs']
  }
